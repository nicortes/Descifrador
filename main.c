...
/**
 * Verifica si el texto es entendible.
 * @param mensaje Texto a verificar.
 * @return 1 si es entendible, 0 si no.
 */
...
/**
 * Imprime los nombres de los integrantes del grupo.
 */
...
int main(int argc, char** argv){
	...
	exit(0);
        ...
        if(strcmp(opcion,"-f") == 0){
            ...
            /*char mensaje[4000];
            input_file = fopen(ruta_archivo,"r");
            if (input_file) {
                //while (fscanf(input_file, "%s", mensaje)!=EOF)
                fscanf(input_file, "%s", mensaje);
                //printf("%s",mensaje);
                //mensaje += str;
                fclose(input_file);
            }
                ...
                printf("No se ha podido abrir el archivo %s",ruta_archivo);
            }*/
